...
info "Performing secure NVMe sanitize..."
# Motive: Use the more forensically secure 'sanitize' command.
...
if nvme sanitize "$DISK" -s 1 -a 2; then
    info "NVMe sanitize completed successfully."
else
    warning "NVMe sanitize failed or is not supported. Falling back to blkdiscard."
    blkdiscard -f "$DISK" || error "Failed to wipe disk with blkdiscard."
fi
